...
func Usage() {
	u_http := UsageSection{
		...
		Hidden:        false,
		ExpectedFlags: []string{"cc", "ck", "H", "X", "b", "d", "r", "u", "opaque", "raw", "recursion", "recursion-depth", "recursion-strategy", "replay-proxy", "timeout", "ignore-body", "x", "sni", "http2"},
	}
	u_general := UsageSection{
		...
		Flags:         make([]UsageFlag, 0),
		Hidden:        false,
		ExpectedFlags: []string{"ac", "acc", "ack", "ach", "acs", "c", "no-content-length", "method-as-raw-request", "config", "json", "maxtime", "maxtime-job", "noninteractive", "p", "rate", "scraperfile", "scrapers", "search", "s", "sa", "se", "sf", "t", "v", "V"},
	}
	...
	u_input := UsageSection{
		...
		Flags:         make([]UsageFlag, 0),
		Hidden:        false,
		ExpectedFlags: []string{"D", "enc", "ic", "input-cmd", "input-num", "input-shell", "mode", "request", "request-proto", "request-keepalive", "e", "w"},
	}
